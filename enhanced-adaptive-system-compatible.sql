-- 🌸 CertBloom Enhanced Adaptive System - Fixed for Existing Schema
...
-- COMPATIBILITY NOTES
-- ============================================================================
-- Your existing questions table uses:
-- - certification_id (UUID) -> we'll join with certifications.name
-- - topic_id (UUID) -> we'll use topics.name as "domain"
-- - No direct concept field -> we'll use 'General' as placeholder
-- 
-- This provides a bridge until you decide to extend the schema

-- ============================================================================
-- ENHANCED USER PROGRESS TRACKING (Compatible Version)
-- ============================================================================
...
CREATE TABLE IF NOT EXISTS user_concept_progress (
    ...
    certification_area TEXT NOT NULL, -- Will store certification.name
    domain TEXT NOT NULL, -- Will store topic.name for now
    concept TEXT NOT NULL, -- 'General' for now, can be refined later
...
-- ============================================================================
-- ADAPTIVE FUNCTIONS (Compatible with Existing Schema)
-- ============================================================================

-- Function to automatically update mastery level
...
BEGIN
    ...
    -- Calculate recent performance (simplified for compatibility)
    SELECT COALESCE(
...
DROP TRIGGER IF EXISTS trg_update_concept_mastery ON user_concept_progress;
...
-- Function to update progress after each question attempt
...
BEGIN
    ...
    SELECT c.name, COALESCE(t.name, 'General')
    ...
    -- If we couldn't find the certification, skip the update
    IF cert_name IS NULL THEN
        RETURN NEW;
    END IF;
    
    -- Check if this continues a streak (simplified)
    SELECT 
...
DROP TRIGGER IF EXISTS trg_update_progress_after_enhanced_attempt ON enhanced_question_attempts;
...
-- ============================================================================
-- INTELLIGENT QUESTION SERVING (Compatible Version)
-- ============================================================================

-- Function to get intelligently selected questions
...
BEGIN
    ...
    available_questions AS (
        SELECT 
            ...
            -- Simple priority based on difficulty and user focus
            CASE 
    ...
    scored_questions AS (
        SELECT 
            ...
            CASE 
                ...
                WHEN aq.priority_score = 3 THEN 'Challenge question for advanced practice'
...
-- Function for user analytics (compatible version)
...
LEFT JOIN topics t ON q.topic_id = t.id
LEFT JOIN enhanced_question_attempts eqa ON q.id = eqa.question_id
GROUP BY c.name, t.name, q.difficulty_level
ORDER BY success_rate_percent ASC;

-- View for learning pattern insights
...
DO $$
BEGIN
    RAISE NOTICE '🌸 CertBloom Enhanced Adaptive System installed successfully!';
    RAISE NOTICE '';
    RAISE NOTICE 'Key Features Available:';
    RAISE NOTICE '✨ Concept-level progress tracking';
    RAISE NOTICE '🎯 Intelligent question selection';
    RAISE NOTICE '📊 Enhanced learning analytics';
    RAISE NOTICE '🌱 Adaptive mastery assessment';
    RAISE NOTICE '';
    RAISE NOTICE 'Next Steps:';
    RAISE NOTICE '1. Test adaptive question selection';
    RAISE NOTICE '2. Record some question attempts';
    RAISE NOTICE '3. Watch progress tracking in action';
    RAISE NOTICE '';
    RAISE NOTICE 'Ready to serve personalized learning! 🌻';
END $$;
